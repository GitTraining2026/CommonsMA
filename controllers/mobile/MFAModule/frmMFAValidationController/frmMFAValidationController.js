  ...
  return {
    ...
    setFormUI: function (mfaResponse) {
      ...
      if (mfaResponse.response.MFAAttributes.MFAType === "SECURE_ACCESS_CODE") {
        scopeObj.view.SecurityCodeComponent.objectServiceName1 =
          mfaResponse.objectServiceDetails.serviceName;
        scopeObj.view.SecurityCodeComponent.objectName1 =
          mfaResponse.objectServiceDetails.dataModel;
        scopeObj.view.SecurityCodeComponent.operationName1 =
          mfaResponse.objectServiceDetails.operationName;
        scopeObj.view.SecurityCodeComponent.objectServiceName2 =
          mfaResponse.objectServiceDetails.serviceName;
        scopeObj.view.SecurityCodeComponent.objectName2 =
          mfaResponse.objectServiceDetails.dataModel;
        scopeObj.view.SecurityCodeComponent.operationName2 =
          mfaResponse.objectServiceDetails.operationName;
        ...
        scopeObj.view.SecurityQuestionsComponent.objectServiceName1 =
          mfaResponse.objectServiceDetails.serviceName;
        scopeObj.view.SecurityQuestionsComponent.objectName1 =
          mfaResponse.objectServiceDetails.dataModel;
        scopeObj.view.SecurityQuestionsComponent.operationName1 =
          mfaResponse.objectServiceDetails.operationName;
    ...
    setFlowActions: function () {
      ...
      scopeObj.view.SecurityCodeComponent.rememberDeviceRegFlag = function (
        rememberDeviceReg
      ) {
        // DO NOTHING
      };

      scopeObj.view.SecurityCodeComponent.onSuccessCallback = function (
        response
      ) {
        applicationManager
          .getPresentationUtility()
          .MFA.navigateToAckScreen(response);
      };

      scopeObj.view.SecurityCodeComponent.onFailureCallback = function (
        response
      ) {
      ...
      scopeObj.view.SecurityQuestionsComponent.rememberDeviceRegFlag =
        function (response) {
          // DO NOTHING
        };

      scopeObj.view.SecurityQuestionsComponent.onSuccessCallback = function (
        response
      ) {
        applicationManager
          .getPresentationUtility()
      ...
      scopeObj.view.SecurityQuestionsComponent.onFailureCallback = function (
        response
      ) {
        scopeObj.setErrorMessageAndLogout(response);
      };
    ...
    setErrorMessageAndLogout: function (response) {
      if (response.hasOwnProperty("isLogoutUser") && response.isLogoutUser) {
        let loginData = applicationManager
          .getNavigationManager()
          .getCustomInfo("frmLoginToast");
        loginData = loginData ? loginData : {};
        loginData.toastMessage = response.errorMessage;
        applicationManager
          .getNavigationManager()
          .setCustomInfo("frmLoginToast", loginData);
        var authMod = kony.mvc.MDAApplication.getSharedInstance()
          .getModuleManager()
          .getModule({
